        ...
        if self.mode == 'camera':
            ...
            try:
                ...
                if not self.cap or not self.cap.isOpened():
                    self.get_logger().error("Could not open camera device 0. Falling back to DEMO mode.")
                    self.mode = 'demo'
            except Exception as e:
                self.get_logger().error(f"Error during camera/MediaPipe initialization: {e}. Falling back to DEMO mode.")
                self.mode = 'demo'
                
        if self.mode == 'demo':
            self.get_logger().info("Running in high-fidelity DEMO mode. Predefined motion loop active.")
        ...
        if self.mode == 'camera' and self.cap is not None:
            ...
            if not self.headless:
                try:
                    ...
                    self.get_logger().warning(f"Could not show cv2 window: {e}. Switching to headless.")
                    self.headless = True
